...
public final class CRCCheck {
	...
	private final static int crc16TabE[] = {
	...
	public static int crc16E(byte[] bufIn, int charCount)
	{
		return crc16Check2(bufIn, charCount);
	}

	private static int crc16Check2(byte[] bufIn, int charCount)
	{
		int crc16;
		int i = 0;
		...
		for(crc16 = 0; charCount > 0; charCount--)
		{
			crc16 = (crc16 << 0x0008) ^ crc16TabE[((crc16 >> 0x0008) ^ bufIn[i++]) & 0x00FF] ;
		}
  ...
  for(i = 0; i < len; i++){
    ...
    for(j = 0; j < 8; j++){
      if(crc & 0x8000){
        crc = (crc << 1) ^ 0x1021;   //여기 0x1021
      }else{                            //역의 방향으로 구현할 때 0x8408이 나오는 것을 볼 수 있다.
        crc = crc << 1;
      }
    }
  }
  ...
  return crc;         //(crc & 0xFFFF)
	 ...
	 * unsigned short calculateCRC2(unsigned char *crcdata, unsigned int len)

{
	...
	public static String makeCRC16_CCITT( String hexString, boolean isReverse) throws Exception {
		   ...
		   if( len % 2 != 0 ) {
		      throw new Exception("HEX format exception 1");
		   }
		   ...
		   for( int i=0; i<input.length; i+=2 ) {
		      ...
		      if( val1 < 0 || val2 < 0 ) {
		          throw new Exception("HEX format exception 2");
		      }
